...
from .forms import OrderForm, RegisterForm, ReviewForm , VendorRegistrationForm
...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is not None:
            login(request, user)
            return redirect(next_url)  # 👈 Redirect to products page
        else:
...
def add_product(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # optionally set supplier based on logged in user
...
def vendor_profile(request, vendor_id):
    ...
    products = Product.objects.filter(supplier__vendor=vendor)
    return render(request, 'vendor_profile.html', {'vendor': vendor, 'products': products})


def vendor_profile(request, vendor_id):
    vendor = get_object_or_404(Vendor, pk=vendor_id)
    products = Product.objects.filter(supplier__vendor=vendor)  # adjust if your relation differs
    return render(request, 'vendor_profile.html', {'vendor': vendor, 'products': products})

@login_required
def vendor_dashboard(request):
    # Assuming each vendor is linked to a user (adjust if your model differs)
    try:
        vendor = Vendor.objects.get(user=request.user)
    except Vendor.DoesNotExist:
        # Redirect or show error if user is not a vendor
        return render(request, 'vendor/not_vendor.html')

    products = Product.objects.filter(supplier__vendor=vendor)
    orders = Order.objects.filter(product__supplier__vendor=vendor)
...
def add_product(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            product = form.save(commit=False)
            product.supplier = vendor  # assuming vendor is supplier or you can adjust here
            product.save()
            return redirect('vendor_dashboard')
    else:
        form = ProductForm()
...
def vendor_dashboard(request):
    ...
    products = Product.objects.filter(supplier=vendor)
